...
class ADMG_RKHSDagma(nn.Module):
    ...
    def get_theta_params(self):
        """Get parameters related to theta (alpha and beta)"""
        theta_params = [
            {'params': self.alpha},
            {'params': self.beta}
        ]
    ...
    def get_sigma_params(self):
        """Get parameters related to Sigma (M)"""
        sigma_params = [
            {'params': self.M}
        ]
        return sigma_params
...
class RKHS_discovery:
    ...
    def minimize(self, 
        ...
        theta_params = self.model.get_theta_params()
        ...
        for i in range(max_iter // 2, max_iter):
        ...
        self.model.alpha.requires_grad_(True)
        self.model.beta.requires_grad_(True)
        return True
